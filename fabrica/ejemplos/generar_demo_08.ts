/**
 * R7-30 (prompt "Construí una máquina, no piezas sueltas", secciones
 * 10+19+20): PRUEBA REAL OBLIGATORIA -- mismo guion/voz/estructura que
 * fabrica-demo-07 (Tomás, curso online) -- experimento A/B real, misma
 * disciplina que ya usó demo_07 contra demo_06: la ÚNICA variable que
 * cambia es que CADA unidad pasa por `aplicarConfiguracionVideo(ctx,
 * 'financiero_directo')` (fabrica/directores/edicion/configuracion.ts,
 * R7-29) en vez de estilos sueltos a mano -- cierra el hueco C)
 * IMPLEMENTADO PERO NO USADO que la autocrítica de la ronda anterior
 * había reconocido explícitamente (ningún generador llamaba a
 * configuracion.ts todavía).
 *
 * `financiero_directo` = ['data', 'documental'] -- basado en el
 * hallazgo real de R7-28 sobre el nicho ("priorización explícita de la
 * CLARIDAD por sobre el estilo visual"), que es exactamente el tipo de
 * guion de este demo (cifras de ventas de un curso online).
 *
 * Uso:
 *   npx tsx ejemplos/generar_demo_08.ts             (genera + escribe el arbol)
 *   npx tsx ejemplos/generar_demo_08.ts --confirmar  (registra en memoria Y datos/, DESPUES del render+QA)
 */
...
const PUBLIC_DEMO_DIR = path.join(RAIZ, 'remotion-spike/public/fabrica_demo_08');
const PUBLIC_MUSICA_DIR = path.join(RAIZ, 'remotion-spike/public/musica');
const BRIDGE_JSON = path.join(RAIZ, 'remotion-spike/src/fabrica_bridge/demo_08.json');
const RUTA_TEMPORAL_CICLO = path.join(RAIZ, 'fabrica/laboratorio/.tmp_demo_08.json');
const RUTA_MP4_ESPERADA = path.join(RAIZ, 'fabrica/salidas/fabrica-demo-08.mp4');
const VIDEO_ID = 'fabrica-demo-08';
...
function clipReal(id: string): ClipAudio {
  ...
  return {archivo: `fabrica_demo_08/${id}.wav`, duracionSeg};
}
...
export function generarArbol(params: ParametrosGeneracion, log: boolean = true): ArbolComposicion {
  ...
  function planificarEdicion(ctx: Omit<ContextoUnidad, 'unidadId'> & {unidadId: string}, golpe: TipoGolpe) {
    ...
    const estrategia = directorEdicion.planificar(ctxConPerfil, golpe, patronesRetencionUsadosEnEsteVideo);
