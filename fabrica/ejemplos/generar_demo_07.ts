/**
 * R7-22: mismo guion/voz/edicion que fabrica-demo-06 (Tomás, curso
 * online) -- a proposito, no es pereza: es un experimento A/B real
 * (fabrica/memoria/tipos.ts, HIPOTESIS/VARIABLE/RESULTADO) donde la
 * UNICA variable que cambia es la musica de fondo, para poder comparar
 * el perfil de audio (QA duro) de demo_06 (sin musica) contra este
 * (con musica) sin que un guion distinto contamine la comparacion.
 *
 * Ejercita, con datos reales, lo que quedaba pendiente desde Ronda 1
 * (`fabrica/musica/README.md`, "Por que no esta conectado todavia"):
 * ahora que `fabrica/musica/biblioteca.json` tiene 9 tracks CC0 reales
 * (R7-22), se llama de verdad a `resolver_musica.py` con la intensidad
 * PROMEDIO de todas las decisiones reales de DirectorAudio en este
 * video (`intensidadMusicaPromedio`, fabrica/directores/audio.ts), y
 * el resultado se adjunta a `arbol.musicaFondo`.
 *
 * Uso:
 *   npx tsx ejemplos/generar_demo_07.ts             (genera + escribe el arbol)
 *   npx tsx ejemplos/generar_demo_07.ts --confirmar  (registra en memoria, DESPUES del render+QA)
 */
...
const AUDIO_ORIGEN = path.join(RAIZ, 'capturas_voz/audio_demo_06'); // reutiliza la voz real de demo_06 a proposito, ver docstring
const PUBLIC_DEMO_DIR = path.join(RAIZ, 'remotion-spike/public/fabrica_demo_07');
const PUBLIC_MUSICA_DIR = path.join(RAIZ, 'remotion-spike/public/musica');
const BRIDGE_JSON = path.join(RAIZ, 'remotion-spike/src/fabrica_bridge/demo_07.json');
const RUTA_TEMPORAL_CICLO = path.join(RAIZ, 'fabrica/laboratorio/.tmp_demo_07.json');
const VIDEO_ID = 'fabrica-demo-07';

mkdirSync(PUBLIC_DEMO_DIR, {recursive: true});
mkdirSync(PUBLIC_MUSICA_DIR, {recursive: true});

/** R7-22: consulta real a fabrica/musica/resolver_musica.py (mismo
 * patron execSync que buscarFondo() de abajo para assets/resolver.py)
 * -- UNA sola vez por video, con la intensidad PROMEDIO de todas las
 * decisiones reales de audio (no por unidad, ver
 * fabrica/musica/README.md). Copia el mp3 real a
 * remotion-spike/public/musica/ (carpeta compartida entre videos,
 * como public/sfx/) solo si todavia no esta ahi. */
function resolverMusicaFondo(decisiones: DecisionAudio[], duracionTotalSeg: number) {
  ...
  try {
    ...
    return {
      ...
      volumen: 0.12, // bajo a proposito: sin ducking dinamico todavia, no puede competir con la narracion
...
function clipReal(id: string): ClipAudio {
  ...
  return {archivo: `fabrica_demo_07/${id}.wav`, duracionSeg};
}
...
export function generarArbol(params: ParametrosGeneracion, log: boolean = true): ArbolComposicion {
  ...
  const decisionesAudio: DecisionAudio[] = []; // R7-22: para intensidadMusicaPromedio()
  ...
  // R7-22: musica de fondo, UNA sola eleccion para todo el video --
  // ver resolverMusicaFondo() y fabrica/musica/README.md.
...
export function confirmarRegistro() {
  ...
  const hipotesis = agregarHipotesis({
    ...
    hipotesis: 'Agregar musica de fondo real (biblioteca CC0, una eleccion por video segun la intensidad ' +
      'promedio del Director de Audio, volumen fijo bajo sin ducking) al mismo guion/voz/edicion que ' +
      'fabrica-demo-06 no debería empeorar el perfil de audio medido por QA duro (silencios/pantallas ' +
      'negras/volumen) -- es la unica variable que cambia respecto de demo_06 (experimento A/B real).',
    experimento: `${VIDEO_ID}: mismo guion/voz que fabrica-demo-06 (Tomás, curso online), UNICA variable ` +
      'nueva: musica de fondo real resuelta por fabrica/musica/resolver_musica.py (R7-22).',
    videoId: VIDEO_ID,
  });
